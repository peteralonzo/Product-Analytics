CREATE OR REPLACE TRANSIENT TABLE USER_DB.AD1_PA08042.HOME_DRIPS
AS
(
WITH home_drips AS
(
...
/* Excludes home policies that are both Control and Test */
exclude_dups AS
(
    SELECT POL_ID, AVG(EXCLUDE_TEST) AS CNT FROM home_drips
    GROUP BY POL_ID
...
hp_filtered AS
(
SELECT * FROM hp_labeled WHERE home_rn = 1
),
...
SELECT fhd.POL_ID,
fhd.CTRL_TEST_CD,
fhd.BILL_INSTAL_TYP_DESC,
hp.POL_EFF_DT,
hp.CANCEL_FLAG,
DATE(hp.POL_TERM_LATST_TRANS_TMSP) AS LATST_TRANS_DT,
hp.TRANS_TYP_DESC,
CASE WHEN
...
hp.ATR_E_FLAT,
hp.CANCEL_E_FLAT
FROM final_home_drips AS fhd
JOIN hp_filtered AS hp
ON fhd.POL_ID = hp.POL_ID
JOIN billing_data bd
ON hp.POL_ID = bd.POL_ID AND hp.POL_EFF_DT = bd.POL_EFF_DT
)
